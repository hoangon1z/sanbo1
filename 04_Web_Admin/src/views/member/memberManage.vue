<template>
    ...
    <Card>
      ...
      <Modal
          class="auditModel"
          v-model="editLevelModal"
          title="会员等级修改"
          @on-ok="editLevel"
          >
         <SPan>当前会员等级:{{levelName}}</SPan>
         <br/>  
         <SPan>选择新等级:</SPan>  
         
           <Select v-model="newLevel" style="width:100px" >
...
import { memberManage, forbiddenMember, forbiddenMemberTrans,getAllLevelValue,updateMemberLevel   } from '@/service/getData';
import {acceptorTrans} from '@/service/getData';
...
export default {
      ...
      columns_first: [
        ...
        },
        {
        ...
        },
         {
          ...
          width: 140,
           render: (h, obj) => {
            let levelName = obj.row.level.name;
            var actions = [];
            actions.push(
                h("span",
                    levelName
                ...
                h("Button",
                  {
                    ...
                    on: {
                      click: () =>{this.handleEditLevel(obj.row)} 
                    },
                  },
        ...
        },
        {
          ...
          render: (h, obj) => {
                    ...
                    else if(name === 'acceptor') {
											acceptorTrans({memberId: obj.row.id})
										.then(res => {
												if(!res.code) {
													this.$Message.success(res.message)
												}else this.$Message.error(res.message)
  ...
  methods: {
     handleEditLevel(row){
      this.editLevelModal = true
      this.levelName =row.level.name
      this.editMemberId =row.id
    },
    refreshPageManual() {
    ...
    editLevel(){
      let memberId = this.editMemberId
      let levelId = this.newLevel 
      updateMemberLevel({memberId:memberId,levelId:levelId}).then(res =>{
          this.refreshPage()
