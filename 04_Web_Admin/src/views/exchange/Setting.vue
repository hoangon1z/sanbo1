    ...
    <Card>
      ...
      <Modal
       ...
       <Modal
          class="auditModel"
          v-model="robotModel"
          :title="'('+(robotParams.robotType==0?'一般机器人':'控盘机器人')+')'+'修改机器人参数'"
          ...
          <ul>
            ...
            <li v-if="robotParams.robotType==2">
                <span><i>*</i>控盘默认价格：</span>
                <p> <Input v-model="robotParams.controlDefaultPrice"></Input> </p>
            </li>
            <li v-if="robotParams.robotType==2">
                <span><i>*</i>价格浮动率：</span>
                <p> <Input v-model="robotParams.controlPriceRate"></Input> </p>
            </li>
            <li>
                <span><i>*</i>最低交易量：</span>
  ...
  data() {
    return {
      ...
      robotParams: {
        ...
        runTime: 0,
        type: 0,
        controlDefaultPrice: 1,
        controlPriceRate: 0.05
      },
      defaultRobotParams: {
        symbol: "",
        isHalt: "1",
                ...
                "Button",
                  {
                    ...
                    on: {
                      click: () => {
                        this.$Loading.start();
                        if(row.robotType == 0 || row.robotType == 2){
                        ...
                        getRobotConfig(obj).then( res => {
                          ...
                          if (!res.code) {
                            if(row.robotType == 0 || row.robotType == 2){
                              ...
                              this.robotParams.robotType = res.data.type;
                              this.robotParams.controlDefaultPrice = res.data.controlDefaultPrice;
                              this.robotParams.controlPriceRate = res.data.controlPriceRate;
                            ...
                            if(row.robotType == 1){
                              ...
                              this.robotParams.controlDefaultPrice = 1;
                              this.robotParams.controlPriceRate = 0;
