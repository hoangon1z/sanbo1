...
import { helpManageDetail, announceDetail, activityDetail, envelopeDetail, miningDetail } from '@/service/getData';
  ...
  mounted () {
    this.$nextTick(() => {
      ...
      if (!!queryDetailId) {
        ...
        }else if (parentComp == 'mining') {
          miningDetail(queryDetailId)
          .then(res => {
              setStore('smeditor', res.data.content);
              document.getElementById('input-area').innerHTML = getStore('smeditor') || '';
          })
