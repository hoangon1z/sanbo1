<template>
  <div class="ctc">
    <img class="bannerimg shoujiIf" src="../../assets/images/ctc-bg.jpg">
                        ...
                        <div class="bd bd_limited">
                          <Form ref="formValidate">
                            ...
                            <FormItem class="trade-input">
                              ...
                              <InputNumber style="width:70%;float:right;" v-model="sellAmount" size="large" :max="50000" :min="2" :placeholder="$t('ctc.input2tips')"></InputNumber>
                              <label class="after">USDT</label>
                            </FormItem>
    ...
    <Modal v-model="modal" width="450">
      ...
      <Form class="withdraw-form-inline" ref="formInline" :model="formInline" inline>
        ...
        <FormItem prop="code">
          <Input id="verifyCode" style="width:calc(100% - 105px)" type="text" autocomplete="off" v-model="formInline.code" :placeholder="$t('uc.regist.smscode')">
          </Input>
...
export default {
  ...
  methods: {
    ...
    getWallet() {
      this.$http
        .post(this.host + this.api.uc.wallet + "USDT", {})
        .then(response => {
          var resp = response.body;
    ...
    valid(type){
      ...
      if(type == 0){
        ...
        if(this.sellAmount == "" || this.sellAmount == null || this.sellAmount == undefined){
          this.$Message.error("请输入卖出数量");
          return false;
        }
        if(this.sellAmount < 2 || this.sellAmount > 50000) {
