  ...
  <div class="login_form">
    <div style="width:100%;text-align:center;font-size:24px;color:#fff;position:absolute;top:80px;">
      <marquee width=100% behavior=scroll direction=left align=middle>
        <span v-for="(item, index) in myRandomly" :key="index" style="margin-right:40%;color:#F3BB01;font-size:20px;">
          {{$t('common.welcome')}}{{item.telNum}}  {{$t('common.become')}}{{item.memberLevelName}}{{$t('common.andReceiveRewards')}}
        </span>
      </marquee>
    </div>
    <div class="login_right">
      ...
      <Alert v-else type="warning">
        Coming soon!
        <template slot="desc">
          KICKSTARTERKIT.COM will open register soon
        </template>
      </Alert>
    </div>
  </div>
...
export default {
  data() {
    ...
    return {
      ...
      ruleInline: {
        ...
        //   {
        //     required: true,
        //     message: this.$t("uc.regist.invitationCode"),
        //     trigger: "blur"
        //   }
        // ],
      },
  ...
  methods: {
    getRandomly() {
      console.log('123123')
      this.$http
        .post(this.host + this.api.uc.getRandomly)
        .then(response => {
          var resp = response.body;
          console.log(resp)
          if (resp.code == 0) {
            this.myRandomly = resp.data;
          }
        });
    },
    updateLangData() {
