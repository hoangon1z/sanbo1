          ...
          >
            <div class="order-handler">
              ...
              <router-link
                ...
                >{{ $t("common.more") }}</router-link
              >
              <!-- <span @click="changeOrder('transactions')" :class="{active:selectedOrder==='transactions'}">{{$t('swap.assetsdetail')}}</span> -->
            </div>
            <div class="table">
              <Table
            ...
            <div class="open" style="text-align: center; margin-top: 10px">
              ...
              <Form style="width: 90%; margin: 0 auto; margin-top: 18px">
                ...
                <FormItem style="margin-bottom: 18px">
                  ...
                  <!-- <Slider class="silder-buy" v-model="form.limitPercent" :step="25" show-tip="always" :tip-format="tipFormat"></Slider> -->
                </FormItem>
              </Form>
      ...
      <div class="left plate-wrap" style="position: relative; flex: 0 0 13%">
        ...
        >
          ...
          <div class="table swap-my-account">
            ...
            <!-- <div class="account-item"> <div>{{$t("swap.principalRate")}}</div>   <div><span>0.0000</span></div></div> -->
          </div>
        </div>
      </div>
    </div>
...
export default {
  ...
  methods: {
    xxlever(value) {
      console.log(value);
      if (value == 1) {
        this.form.leverageAdjustVal = 1;
      } else if (value == 2) {
        this.form.leverageAdjustVal = 2;
      } else if (value == 3) {
        this.form.leverageAdjustVal = 5;
      } else if (value == 4) {
        this.form.leverageAdjustVal = 10;
      } else if (value == 5) {
        this.form.leverageAdjustVal = 20;
      }
    },
    xxlever2(value) {
      console.log(value);
      if (value == 1) {
        this.form.timeAdjustVal = 10;
      } else if (value == 2) {
        this.form.timeAdjustVal = 30;
      } else if (value == 3) {
        this.form.timeAdjustVal = 60;
      } else if (value == 4) {
        this.form.timeAdjustVal = 120;
      } else if (value == 5) {
        this.form.timeAdjustVal = 360;
      } else if (value == 6) {
        this.form.timeAdjustVal = 3000;
      }
    },
    getLeverage() {
      this.$http
        .get(
          this.host +
            this.api.swap.leverage +
            "?symbol=" +
            this.currentCoin.symbol
        )
        .then((response) => {
          var resp = response.body;
          this.form.leverage = resp.data.leverage;
          if (resp.data.leverage == 1) {
            this.form.leverageAdjustVal2 = 1;
          } else if (resp.data.leverage == 2) {
            this.form.leverageAdjustVal2 = 2;
          } else if (resp.data.leverage == 5) {
            this.form.leverageAdjustVal2 = 3;
          } else if (resp.data.leverage == 10) {
            this.form.leverageAdjustVal2 = 4;
          } else if (resp.data.leverage == 20) {
            this.form.leverageAdjustVal2 = 5;
          }
        });
    },
    quickClose(order) {
      ...
      this.$Modal.confirm({
        ...
        content: this.$t("swap.closetip") + order.availablePosition,
        onOk: () => {
          this.$http
            .post(this.host + this.api.swap.quickClose + order.id)
            .then((response) => {
              var resp = response.body;
              if (resp.code == 0) {
                this.$Notice.success({
    ...
    closeOrder(direction) {
      if (direction == 1 && this.form.limitAmount > this.maxCloseBuyAmount) {
        this.$Notice.error({
          title: this.$t("swap.tip"),
          desc: this.$t("swap.buyamounttipwarning") + this.maxCloseBuyAmount,
        });
        return;
      }
      if (direction == 2 && this.form.limitAmount > this.maxCloseSellAmount) {
        this.$Notice.error({
          title: this.$t("swap.tip"),
          desc: this.$t("swap.sellamounttipwarning") + this.maxCloseSellAmount,
        });
        return;
      }
      let triggerType =
        this.form.triggerPrice <= this.currentCoin.close ? 1 : 2; //1大于，2小于
      this.$http
        .post(this.host + this.api.swap.addOrderEntrust, {
          contractCoinId: this.currentCoin.id,
          symbol: this.currentCoin.symbol,
          entrustType: this.form.entrustType,
          type: this.form.type,
          patterns: this.form.patterns,
          leverage: this.form.leverage,
          marketPrice: this.form.market ? 1 : 0,
          entrustPrice: this.form.market ? null : this.form.limitPrice,
          triggerPrice: this.form.triggerPrice,
          triggerType: triggerType,
          share: parseInt(this.form.limitAmount),
          direction: direction,
        })
        .then((response) => {
          var resp = response.body;
    ...
    openOrder(direction) {
      if (this.form.limitAmount > this.maxOpen) {
        this.$Notice.error({
          title: this.$t("swap.tip"),
          desc:
            (direction == 1
              ? this.$t("swap.buyamounttipwarning")
              : this.$t("swap.sellamounttipwarning")) + this.maxOpen,
        });
        return;
      }
      //判断手续费是否够用
      ...
      if (
        ...
        parseInt(this.form.limitAmount) *
          this.currentCoin.perUsdt *
          this.takerFee >
          this.wallet.kick
      ) {
        // 买入开仓
      ...
      if (!feeEnable) {
        this.$Modal.confirm({
          title: this.$t("common.feetip"),
          content: this.$t("common.feetipdetail"),
          cancelText: this.$t("common.feeclosetip"),
          onOk: () => {
            this.$router.push("/crowdfunding");
          },
          onCancel: () => {
            this.doOpen(direction);
          },
        });
      } else {
        this.doOpen(direction);
    ...
    init() {
      ...
      this.$http
        ...
        .then((response) => {
          ...
          if (this.currentCoin.type === "ALWAYS") {
            this.$store.commit("navigate", "nav-swapindex");
          } else {
            this.$store.commit("navigate", "nav-secswap");
          }
    ...
    startWebsock() {
      ...
      stompClient.connect({}, function (frame) {
        ...
        if (that.isLogin) {
          ...
          stompClient.subscribe("/topic/swap/refresh/" +
              that.currentCoin.symbol +
              "/" +
              that.member.id, function (msg) {
            //这个替代了以上所有，代表成交，爆仓等信息
            // console.log(msg);
            that.refreshAccount();
          });
        }
