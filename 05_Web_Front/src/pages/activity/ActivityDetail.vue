                    ...
                    <div class="right-container">
                        ...
                        <div class="bottom">
                          <div v-if="(activityDetail.type==3 || activityDetail.type==4 || activityDetail.type==5)">
                            <Button type="warning" size="large" long v-if="activityDetail.step==1 && isActivityInDate" @click="apply">
                              {{$t('activity.attend')}}
                            </Button>
                            <Button type="warning" size="large" long disabled  v-else>{{$t('activity.attend')}}</Button>
                          </div>
                          <div v-else>
                            <Button type="warning" :disabled='screenWidth < 768' size="large" long v-if="activityDetail.step==1 && isActivityInDate" @click="apply">
                              <span >{{screenWidth > 768 ? $t('activity.attend') : $t('activity.login')}}</span>
                            </Button>
                            <Button type="warning" size="large" long disabled  v-else>{{screenWidth > 768 ? $t('activity.attend') : $t('activity.login')}}</Button>
                          </div>
                        </div>
  ...
  methods: {
    ...
    getData(){
        ...
        if (res.status == 200 && res.body.code == 0) {
            ...
            window.document.title = (this.lang == "简体中文" ? "活动 - " : "Activity - ") + this.activityDetail.title + " - KICKSTARTERKIT | 全球比特币交易平台 | 全球数字货币交易平台";
            //持仓瓜分类型进度显示处理
    ...
    attendClick() {
      ...
      // 余额检查
      // if(this.activityDetail.type == 4){
      //     if(parseFloat(this.attendAmount) * this.activityDetail.price * 0.7 > this.mybalance) {
      //         this.$Notice.error({
      //           title: this.$t("common.tip"),
    ...
    getMember() {
      ...
      this.$http
        ...
        .then(response => {
          var resp = response.body;
          if (resp.code == 0) {
            this.myLevel = resp.data.level.id;
