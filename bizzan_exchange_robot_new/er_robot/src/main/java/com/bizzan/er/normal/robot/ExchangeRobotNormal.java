...
public class ExchangeRobotNormal implements Runnable, ExchangeRobot{
	...
	public ExchangeRobotNormal(String sign, String currenOrderURL, String cancelOrderURL, String commitOrderURL, String plateURL) {
		this.sign = sign;
		this.currenOrderURL = currenOrderURL;
		this.cancelOrderURL = cancelOrderURL;
		this.commitOrderURL = commitOrderURL;
	...
	protected int uid = 1;
//	robot.sign=987654321asdf
//	robot.plate.url=http://127.0.0.1/market/exchange-plate-mini
//	robot.current.order.url=http://127.0.0.1/exchange/order/mockcurrentydhdnskd
//	robot.cancel.order.url=http://127.0.0.1/exchange/order/mockcancelydhdnskd
//	robot.commit.order.url=http://127.0.0.1/exchange/order/mockaddydhdnskd
	...
	// 控盘机器人数据
	private ArrayList<RobotQuota> quotaLines = new ArrayList<>();
	...
	@Override
	public void clearQuotaLines() {
		quotaLines = new ArrayList<>();
	}
	...
	public void updateRobotParams(RobotParams params) {
    	...
    	this.robotParams.setControlDefaultPrice(params.getControlDefaultPrice());
    	this.robotParams.setControlPriceRate(params.getControlPriceRate());
...
//			logger.info("发送订单结果:"+result);
		...
		while(true) {
			...
			try {
				...
				if(plateResult != null && !plateResult.equals("")) {
					...
					if (robotParams.getType()==0) {
						lastestPrice = this.getOuterPrice();
					} else {
						lastestPrice = this.getRegulatePrice();
					}
	...
	/**
	 * 重新获取节点(时间乘以10倍，判断时再除)
	 * @param today
	 * @param time
	 */
	private void refetchQuotaLinesLines (String today, long time) {
		long minute = time / 60;
		RobotQuota r1 = robotQuotaService.findBefore(this.plateSymbol, today, minute);
		RobotQuota r2 = robotQuotaService.findAfter(this.plateSymbol, today, minute);
		BigDecimal controlDefaultPrice = robotParams.getControlDefaultPrice();
		if (controlDefaultPrice==null || controlDefaultPrice.compareTo(BigDecimal.ZERO)<=0) {
			controlDefaultPrice = BigDecimal.ONE;
		}
		if (r1==null) {
			r1 = new RobotQuota();
			r1.setTime(time-1);
			r1.setPrice(controlDefaultPrice);
			r1.setDate(today);
			r1.setSymbol(this.plateSymbol);
		} else {
			r1.setTime(r1.getTime()*60);
		}
		if (r2==null) { // 构造60分钟假数据
			r2 = new RobotQuota();
			r2.setTime(time+60*60);// 加60分钟
			r2.setPrice(controlDefaultPrice);
			r2.setDate(today);
			r2.setSymbol(this.plateSymbol);
		} else {
			r2.setTime(r2.getTime()*60);
		}
		quotaLines = new ArrayList<>();
		quotaLines.add(r1);
		int st = r1.getTime().intValue();
		BigDecimal bp = r1.getPrice(); // 上一个价格
		while (st < r2.getTime().intValue()) {
			int slice = MyRandomUtil.getRandom(50, 600); // 取50秒到10分钟随机数
			st += slice;
			if (st >= r2.getTime().intValue()) {
				quotaLines.add(r2);
			} else {
				BigDecimal cp = getLadderPirce(r1, r2, Long.valueOf(st));
				BigDecimal rp = getRandPriceLimintPoints(cp, robotParams.getControlPriceRate());
		...
		System.out.println(robotParams.getControlDefaultPrice());
		System.out.println(robotParams.getControlPriceRate());
	 ...
	 * @return
	 */
	private Map<String, RobotQuota> getSides(String today, long time) {
		if (quotaLines.size()==0) {
			refetchQuotaLinesLines(today, time);
		}
		if (quotaLines.size()!=0 && (quotaLines.get(quotaLines.size()-1).getTime()<time || quotaLines.get(0).getTime()>time)) {
			refetchQuotaLinesLines(today, time);
		}
		RobotQuota before = null;
		RobotQuota after = null;
		if (quotaLines.size() != 0) {
			for (int i=0;i<quotaLines.size();i++) {
				before = quotaLines.get(i);
				if (quotaLines.get(i+1).getTime()>=time) {
					after = quotaLines.get(i+1);
					break;
				}
			}
		}
		HashMap<String, RobotQuota> re = new HashMap<>();
		re.put("before", before);
		re.put("after", after);
		return re;
	}
	...
	public BigDecimal getRegulatePrice() {
		try {
			...
			long time = (now - todayBegin) / 1000  ; //(秒)
			Map<String, RobotQuota> sides = getSides(today, time);
			RobotQuota before = sides.get("before");
			RobotQuota after = sides.get("after");
			if (before==null || after==null) {
				throw new Exception("控盘机器人:未查找到设置数据");
			}
			return getLadderPirce(before, after, time);
	...
	public BigDecimal getLadderPirce(RobotQuota before, RobotQuota after, long time) {
		long btl = time - before.getTime();
		long atl = after.getTime() - time;
		BigDecimal ladder = BigDecimal.valueOf(btl).divide(BigDecimal.valueOf(btl + atl), 8, BigDecimal.ROUND_DOWN);
	...
	public void setRobotQuotaService(RobotQuotaService service) {
		this.robotQuotaService = service;
	}
