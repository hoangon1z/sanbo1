...
//        ExchangeCoin coin = exchangeCoinService.findBySymbol(order.getSymbol());
        BigDecimal fee = amount.multiply(exchangeCoin.getFee());
        MemberWallet feeWallet = memberWalletService.findByCoinUnitAndMemberId("KICK", member.getId());
        if (feeWallet.getBalance().compareTo(fee) < 0) {
            return MessageResult.error("手续费不足，所需：" + fee + "KICK");
        }
        memberWalletService.kickIncreaseBalance(feeWallet.getId(), fee.negate()); //直接扣除手续费
...
//        log.info(">>>>>>>>>>订单提交完成>>>>>>>>>>");
    ...
    public MessageResult cancelOrder(@SessionAttribute(SESSION_MEMBER) AuthMember member, @PathVariable String orderId) {
        ...
        /** kickout 取消委托时退回手续费**/
        BigDecimal fee = (order.getAmount().subtract(order.getTradedAmount())).multiply(exchangeCoin.getFee());
        MemberWallet feeWallet = memberWalletService.findByCoinUnitAndMemberId("KICK", member.getId());
        memberWalletService.kickIncreaseBalance(feeWallet.getId(), fee); //直接返回手续费
        /** kickout end **/
