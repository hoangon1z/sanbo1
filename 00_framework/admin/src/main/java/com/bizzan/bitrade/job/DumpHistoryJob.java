...
//		List<ExchangeOrder> list = exchangeOrderService.queryHistoryDelete(beforeTime);
        int deleteCount = exchangeOrderService.deleteHistory(beforeTime);
        log.info("实际清除交易订单：" + deleteCount + "条");
        log.info("清除指定时间之前的订单详情");
        Query query = new Query(Criteria.where("time").lt(beforeTime));
        mongoTemplate.remove(query, ExchangeOrderDetail.class);

        Date today = new Date();
        Calendar now = Calendar.getInstance();
        now.setTime(today);
        now.set(Calendar.DATE, now.get(Calendar.DATE) - 3); // 清理3天前数据
