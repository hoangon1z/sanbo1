...
public class ActivityController extends BaseAdminController {
    ...
    public MessageResult distribute(@RequestParam("oid") Long oid) {
        ...
        if (activity.getType() == 3) {
            ...
            try {
                smsProvider.sendCustomMessage(member.getMobilePhone(), "尊敬的用户，您参与活动兑换的" + disAmount + "已到账！");
            } catch (Exception e) {
...
// 			}
            ...
            try {
                smsProvider.sendCustomMessage(member.getMobilePhone(), "尊敬的用户，您购买的云端已部署成功！");
            } catch (Exception e) {
                return error(e.getMessage());
            }
