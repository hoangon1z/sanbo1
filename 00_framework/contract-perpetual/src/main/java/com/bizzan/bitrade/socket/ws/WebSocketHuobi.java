...
public class WebSocketHuobi extends WebSocketClient {
    ...
    public WebSocketHuobi(URI uri, ContractCoinMatchFactory matchFactory) {
        super(uri);
        this.uri = uri;
...
//            logger.info("[WebSocketHuobi][" + symbol + "] 深度订阅: " + topic);
    ...
    public void reqKLineList(String symbol, String period, long from, long to) {
        ...
        long divideTime = HuobiTime.getDivideTime(period) * 300; // 1秒 * 300条
                    ...
                    if (type.equals("kline")) {
                        ...
                        if (null != data && !"".equals(data) && JSONUtils.isJsonArray(data)) {
                            ...
                            for (int i = 0; i < klineList.size(); i++) {
                                ...
                                // 可能导致k线出问题，暂时注释
//                                if (!checkNewTime(symbol + "-" + period , time)) {
//                                    continue;
//                                }
...
//                                marketService.saveKLine(symbol, kline);
                                ...
                                // 推送K线(如果只有一条，说明是最新的K线，需要推送到前端K线) 10.13 wmf 实时上并没有返回一条数据的情况因此此处修改为不判断
//                                if(klineList.size() == 1) {
                                this.matchFactory.getContractCoinMatch(symbol).refreshKline(kline);
