...
//    List<Member> findAllByTeamCompute(Boolean team);

    @Procedure(name = "getParentList")
    String findMyParent(@Param("root_id") Long root_id);
    ...
    @Transactional
    @Modifying
    @Query(value = "update member m  set m.member_level_id = :memberId where m.id = :levelId", nativeQuery = true)
    void updateLevelId(@Param("memberId") Long memverId, @Param("levelId") Long levelId);
     ...
     */
    @Modifying
    @Query(value = "update Member set teamLevel = teamLevel + 1 where id in (:idList) ")
    void addOneTeamLevel(@Param("idList") List<Long> idList);
     ...
     */
    @Modifying
    @Query(value = "update Member set teamLevel = teamLevel + 1 where id = :id")
    void addOneTeamLevel(@Param("id") Long id);
     ...
     */
    @Modifying
    @Query(value = "update Member set teamMiningPower = teamMiningPower + :addPower where id = :id")
    void addOneTeamMiningPower(@Param("id") Long id, @Param("addPower") BigDecimal addPower);
}
