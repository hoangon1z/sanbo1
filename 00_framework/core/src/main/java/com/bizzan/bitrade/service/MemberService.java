...
public class MemberService extends BaseService {
     ...
     * @param addPower
     */
    public void addPower(Long memberId, BigDecimal addPower) {
        Member member = findOne(memberId);
        member.setPower(member.getPower().add(addPower));
        member.setTeamPower(member.getTeamPower().add(addPower));
        save(member);
    }
...
//     * @return
//     */
//    public List<Member> findUnInTeamUser() {
//        return memberDao.findAllByTeamCompute(false);
//    }
     ...
     * @return
     */
    public String findMyParent(Member member) {
        return memberDao.findMyParent(member.getId());
    }

    @Transactional
    public void addOneTeamLevel(Long id) {
        memberDao.addOneTeamLevel(id);
    }

    @Transactional
    public void addOneTeamMiningPower(Long id, BigDecimal addPower) {
        memberDao.addOneTeamMiningPower(id, addPower);
    }
    ...
    /**
     * kick 客户需求，每产生100kb手续费，整体增加0.5%算力
     */
    public void addKickFeeHandler(Long id, BigDecimal fee) {
        Member member = findOne(id);
        BigDecimal kickFee = member.getKickFee();
        if (kickFee == null) {
            kickFee = BigDecimal.ZERO;
        }
        BigDecimal[] remainders = kickFee.divideAndRemainder(BigDecimal.valueOf(100));
        BigDecimal remainder = remainders[1]; // 余数
        if ((remainder.add(fee)).compareTo(BigDecimal.valueOf(100)) > 0) {
            // 增加算力
            member.setPower(member.getPower().add(BigDecimal.valueOf(0.005)));
        }
        member.setKickFee(kickFee.add(fee)); // 保存总手续费
        save(member);
    }
    ...
    public void updateMemberLevel(Long memberId, Long levelId) {
        ...
        MemberLevel memberLevel = memberLevelDao.findOne(levelId);
