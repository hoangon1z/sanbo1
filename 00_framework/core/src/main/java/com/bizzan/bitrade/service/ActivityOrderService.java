...
public class ActivityOrderService extends BaseService {
    ...
    public MessageResult saveActivityOrder(Long memberId, ActivityOrder activityOrder) {
        ...
        if (activity.getType() == 4 || activity.getType() == 5) {
            // 新进度不能小于旧进度
            int newProgress = activity.getTradedAmount().divide(activity.getTotalSupply(), 6, BigDecimal.ROUND_DOWN).multiply(new BigDecimal(100)).intValue();
        ...
        /** kickout 增加矿机算力，自由认购 **/
        if (activity.getType() == 4 && activity.getPowerNum() != null && activity.getPowerNum() > 0 && activityOrder.getAmount().compareTo(BigDecimal.valueOf(activity.getPowerNum())) >= 0 && activity.getPower() != null && activity.getPower().compareTo(BigDecimal.ZERO) > 0) {
            BigDecimal powerTimes = activityOrder.getAmount().divide(BigDecimal.valueOf(activity.getPowerNum()), 0, BigDecimal.ROUND_HALF_DOWN);
            BigDecimal addPower = activity.getPower().multiply(powerTimes);
            memberService.addPower(memberId, addPower);
        }
        /** kickout end **/
