...
public class MemberWalletService extends BaseService {
    ...
    public MessageResult recharge(Coin coin, String address, BigDecimal amount, String txid) {
        ...
        if (mRes != null) {
            try {
                smsProvider.sendCustomMessage(mRes.getMobilePhone(), "尊敬的用户：恭喜您充值成功，充值数量为：" + amount.stripTrailingZeros().toPlainString());
            } catch (Exception e) {
                e.printStackTrace();
            }
        }
    ...
    public MemberWallet findByCoinUnitAndMemberId(String coinUnit, Long memberId) {
        ...
        MemberWallet e = findByCoinAndMemberId(coin, memberId);
        if (e == null) {
    ...
    public MemberWallet findByCoinAndMemberId(Coin coin, Long memberId) {
        MemberWallet wallet = memberWalletDao.findByCoinAndMemberId(coin, memberId);
        if (wallet != null) {
            return wallet;
        }
        MemberWallet newWallet = new MemberWallet();
        newWallet.setCoin(coin);
        newWallet.setBalance(BigDecimal.ZERO);
        newWallet.setFrozenBalance(BigDecimal.ZERO);
        newWallet.setMemberId(memberId);
        return memberWalletDao.save(newWallet);
    }
    ...
    /**
     * kick用户专用，为应对控制器直接调用没有事务的情况，请勿使用！！！
     *
     * @param walletId
     * @param amount
     */
    @Transactional
    public void kickIncreaseBalance(Long walletId, BigDecimal amount) {
