...
public class MemberInviteStasticJob {
    ...
    private void updateTeamLevel(Member member) {
        ...
        memberService.addOneTeamLevel(up.getId());
        updateTeamLevel(up);
    }

    /**
     * 每日1点处理，统计用户的团队人数(0 0 1 * * *) todo 暂时弃用
     */
//	@Scheduled(cron = "0 0 1 * * *")
//	@Scheduled(cron = "0 */1 * * * *")
    public void setTeamLevel() {
        log.info("开始计算团队人数");
//		List<Member> members = memberService.findUnInTeamUser();
//		for (Member member:members) {
//			updateTeamLevel(member);
////			member.setTeamCompute(true);
...
//		}
    ...
    /**
     * 每日2点处理，统计用户推广币币手续费返佣结果(0 0 2 * * *)，总榜,此方法暂时没用
     */
                ...
                for (Member item : all) {
                    if (item.getId() >= 10) { // 10以下为机器人
                        ...
                        if (mis != null) {
                            ...
                            mis.setLevelTwo(item.getTeamLevel());
                        ...
                        if (mis != null) {
                            ...
                            mis.setLevelTeam(mis.getLevelTwo() + (rand1 % 20)); // 团队好友人数每天自动增加0-20个
