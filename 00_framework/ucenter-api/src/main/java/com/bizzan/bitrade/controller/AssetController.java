...
public class AssetController {
    ...
    public MessageResult findContractWallet(@SessionAttribute(SESSION_MEMBER) AuthMember member) {
        /**币币钱包余额**/
        List<MemberWallet> memberWallets = walletService.findAllByMemberId(member.getId());
        HashMap<String, BigDecimal> memberWalletsMap = new HashMap<>();
        for (MemberWallet memberWallet : memberWallets) {
            memberWalletsMap.put(memberWallet.getCoin().getName(), memberWallet.getBalance());
        }
        /**合约钱包**/
        ...
        wallets.forEach(wallet -> {
            ...
            wallet.getCoin().setColdWalletAddress(""); // 对前端隐藏冷钱包地址
            BigDecimal mainBalance = memberWalletsMap.get(wallet.getCoin().getName());
            if (mainBalance != null) {
                wallet.setMainBalance(mainBalance);
            }
        });
        MessageResult mr = MessageResult.success("success");
        mr.setData(wallets);
    ...
    @RequestMapping("contract-transaction/all")
    public MessageResult contractDetails(@SessionAttribute(SESSION_MEMBER) AuthMember member, HttpServletRequest request, int pageNo, int pageSize,
                                         @RequestParam(value = "startTime", required = false) String startTime,
                                         @RequestParam(value = "endTime", required = false) String endTime,
                                         @RequestParam(value = "symbol", required = false) String symbol,
                                         @RequestParam(value = "type", required = false) String type) throws ParseException {
        MessageResult mr = new MessageResult();
        ContractTransactionType contractTransactionType = null;
        if (StringUtils.isNotEmpty(type)) {
            contractTransactionType = contractTransactionType.valueOfOrdinal(Convert.strToInt(type, 0));
        }
        mr.setCode(0);
        mr.setMessage("success");
        mr.setData(contractTransactionService.queryByMember(member.getId(), pageNo, pageSize, contractTransactionType, startTime, endTime, symbol));
        return mr;
    }
     ...
     * @return
     */
    @RequestMapping("contract-wallet/{symbol}")
    public MessageResult findContractWalletBySymbol(@SessionAttribute(SESSION_MEMBER) AuthMember member, @PathVariable String symbol) {
        MessageResult mr = MessageResult.success("success");
        ContractWallet wallet = contractWalletService.findByCoinUnitAndMemberId(symbol, member.getId());
        MemberWallet bbWallet = walletService.findByCoinUnitAndMemberId(symbol, member.getId());
        wallet.setMainBalance(bbWallet.getBalance());
        mr.setData(wallet);
        return mr;
    }
     ...
     * @return
     */
    @PostMapping("contract-wallet/transfer")
    public MessageResult transfer(@SessionAttribute(SESSION_MEMBER) AuthMember member, String unit, BigDecimal amount, TransferDirection direction) throws Exception {
        MessageResult mr;
        if (direction == TransferDirection.contract) {
            mr = contractWalletService.transferIn(member.getId(), unit, amount);
        } else {
            mr = contractWalletService.transferOut(member.getId(), unit, amount);
        }
        contractTransactionService.record(member.getId(), unit, amount, direction == TransferDirection.contract ? ContractTransactionType.IN : ContractTransactionType.OUT);
     ...
     * @return
     */
    @GetMapping("contract-wallet/transfer/record")
    public MessageResult pageWithdraw(@SessionAttribute(SESSION_MEMBER) AuthMember user, int pageNo, int pageSize) {
        MessageResult mr = new MessageResult(0, "success");
        Page<Transfer> records = transferService.findAllByMemberId(user.getId(), pageNo, pageSize);
        mr.setData(records);
        return mr;
    }
