...
public class LoginInfo {
    ...
    private int teamLevel = 0; // 团队邀请好友数量

    private BigDecimal power = BigDecimal.ZERO; // 我的算力(kick)

    private BigDecimal teamPower = BigDecimal.ZERO; // 团队算力(kick)
    ...
    public static LoginInfo getLoginInfo(Member member, String token, Boolean signInActivity, String prefix) {
        return LoginInfo.builder().location(member.getLocation())
                ...
                .teamLevel(member.getTeamLevel())
                .level(member.getLevel())
                .power(member.getPower())
                .teamPower(member.getTeamPower())
                .build();
    }
}
