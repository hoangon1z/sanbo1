...
//    }
    ...
    public MessageResult createFreeCard(@SessionAttribute(SESSION_MEMBER) AuthMember member) {
        ...
        card.setCardName("公益币推广卡");
        card.setCardNo(authMember.getPromotionCode() + GeneratorUtil.getNonceString(5).toUpperCase());
        card.setAmount(new BigDecimal(9.9));
        card.setCardDesc("");
        card.setCoin(coinService.findByUnit("KICK"));
        card.setCount(30);
    ...
    private MessageResult exhcangeCard(@SessionAttribute(SESSION_MEMBER) AuthMember member,
        ...
        if (newOrder != null) {
            ...
            if (authMember.getInviterId() == null || authMember.getInviterId().equals(0l)) {
            ...
            // 添加资产到用户钱包
            memberWalletService.kickIncreaseBalance(memberWallet.getId(), newOrder.getAmount());
     ...
     * 随机显示注册用户
     */
    @PostMapping("getRandomly")
    public MessageResult getRandomly() {
        List<RandomlyRegistered> randomlys = registeredService.getRandomly();
        MessageResult result = MessageResult.success("ok");
        result.setData(randomlys);
        return result;
    }
